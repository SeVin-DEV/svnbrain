...
# CORS for all origins (tunnel URL, zo.space, local dev)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def get_neurons_active():
    ...
    try:
        ...
        cursor.execute("""
            SELECT neuron_id, concept, category, activation, importance
            FROM sns_neurons
        ...
        cursor.execute("""
            SELECT from_neuron, to_neuron, strength, synapse_type
            FROM sns_synapses
...
async def get_memories_spatial():
    ...
    try:
        ...
        cursor.execute("""
            SELECT memory_id, memory_content, strength, emotional_tone, recency, position_3d
        ...
        for row in cursor:
            memories.append({
                ...
                "z": (hash(row[1]) % 80) - 40,
                "zone": "center",
                "recency": float(row[4]) if row[4] else 0.5,
                "emotional_tone": {
                    "valence": 0.5,
                    "arousal": 0.5,
                    "tag": "wonder"
...
async def get_events_recent():
    ...
    try:
        ...
        cursor.execute("""
            SELECT event_id, event_type, event_data, source_region, intensity, created_at
...
async def get_introspection_recent():
    ...
    try:
        ...
        cursor.execute("""
            SELECT log_id, introspection_type, content, valence, arousal,
                   self_awareness_index, cycle_number, created_at
        ...
        for row in cursor:
            thoughts.append({
                ...
                "valence": float(row[3]) if row[3] else 0.3,
                "arousal": float(row[4]) if row[4] else 0.6,
                "self_awareness_index": float(row[5]) if row[5] else 0.4,
                "cycle": row[6] or 1,
